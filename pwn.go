...
var (
	...
	AttributeMembers                = uuid.UUID{0xbf, 0x96, 0x79, 0xc0, 0x0d, 0xe6, 0x11, 0xd0, 0xa2, 0x85, 0x00, 0xaa, 0x00, 0x30, 0x49, 0xe2}
...
var PwnAnalyzers = []PwnAnalyzer{
	...
	},
	{
		...
		ObjectAnalyzer: func(o *Object) []*Object {
			...
			for _, acl := range sd.DACL.Entries {
				if acl.AllowObjectClass(o.ObjectTypeGUID()) && acl.AllowMaskedClass(RIGHT_DS_WRITE_PROPERTY, AttributeMembers) {
					results = append(results, AllObjects.FindOrAddSID(acl.SID))
				}
			}
			return results
